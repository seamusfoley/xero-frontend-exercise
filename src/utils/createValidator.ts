...
export const createValidator = (validations: Validation[], cb: AnyFunction) => {
  ...
  const createValidationApplier = <T extends {[key: string]: unknown}>(
    ...
    const thisValidation = Object.entries(formState)
      .reduce<{[key: string]: {validity: Validity, message?: string }}>((a, [key, value]) => {
    ...
    const mergedValidationState = Object.entries(validationState).reduce<ValidationState>((a, [key, value]) => {
      ...
      return ({
        ...
        [key]:{
          ...
          messages: thisValidationField.message ? [...value.messages, thisValidationField.message] : value.messages
        }
      })
    }, {})
  ...
  return <T extends {[key: string]: any}>(formState: T ): ValidationState => {
    const v = validations.map(validation => createValidationApplier(formState, validation))

    const initialValidationState = Object.entries(formState)
      .reduce<ValidationState>((a, [key, _]) => (
        {...a, [key]: {
